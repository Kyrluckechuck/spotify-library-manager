...
class ArtistInput(BaseModel):
    ...
    gid: str = Field(..., min_length=1, max_length=100, description="Spotify artist GID")
...
class AlbumInput(BaseModel):
    ...
    spotify_gid: str = Field(..., min_length=1, max_length=100, description="Spotify album GID")
    spotify_uri: str = Field(..., min_length=1, max_length=100, description="Spotify album URI")
    ...
    @classmethod
    def validate_spotify_gid(cls, v):
        """Validate Spotify GID format."""
...
class SongInput(BaseModel):
    ...
    failed_count: int = Field(..., ge=0, description="Number of failed download attempts")
    ...
    spotify_uri: str = Field(..., min_length=1, max_length=100, description="Spotify URI")
    artist: Optional[str] = Field(None, max_length=500, description="Artist name")
...
class TaskHistoryInput(BaseModel):
    ...
    task_id: str = Field(..., min_length=1, max_length=100, description="Internal task ID")
...
class UpdatePlaylistInput(MutationInput):
    ...
    name: Optional[str] = Field(None, min_length=1, max_length=500, description="Playlist name")
...
class ArtistsQueryInput(QueryInput):
    ...
    sort_direction: Optional[str] = Field(None, pattern="^(asc|desc)$", description="Sort direction")
...
class SongsQueryInput(QueryInput):
    ...
    artist_id: Optional[int] = Field(None, gt=0, description="Filter by artist ID")
    downloaded: Optional[bool] = Field(None, description="Filter by downloaded status")
    unavailable: Optional[bool] = Field(None, description="Filter by unavailable status")
    first: int = Field(default=20, ge=1, le=100, description="Number of items to fetch")
    after: Optional[str] = Field(None, description="Cursor for pagination")
    sort_by: Optional[str] = Field(None, max_length=50, description="Sort field")
    sort_direction: Optional[str] = Field(None, pattern="^(asc|desc)$", description="Sort direction")
