...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        if show_stats:
            ...
            self.stdout.write(f'  Average logs per task: {stats["average_logs_per_task"]}')
            self.stdout.write('')
        
        if dry_run:
            self.stdout.write(f'DRY RUN - Would delete completed/failed tasks older than {days_to_keep} days')
        ...
        if old_count == 0:
            self.stdout.write(
                self.style.SUCCESS(f'No completed/failed tasks older than {days_to_keep} days found')
            )
            return
        ...
        if dry_run:
            ...
            self.stdout.write(
                self.style.SUCCESS(f'Deleted {deleted_count} old tasks (older than {days_to_keep} days)')
            ) 