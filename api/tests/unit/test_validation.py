...
from api.library_manager.validation import (
    validate_spotify_url, extract_spotify_id, is_spotify_playlist_url,
    is_spotify_album_url, is_spotify_track_url, get_spotify_url_type
)
...
class TestValidationFunctions:
    ...
    def test_extract_spotify_id_valid(self):
        ...
        test_cases = [
            ...
            ('https://open.spotify.com/artist/0TnOYISbd1XYRBk9myaseg', '0TnOYISbd1XYRBk9myaseg'),
    ...
    def test_extract_spotify_id_invalid(self):
        ...
        for url in invalid_urls:
            extracted_id = extract_spotify_id(url)
            assert extracted_id is None, f"Should return None for {url}, got {extracted_id}"
    ...
    def test_is_spotify_playlist_url(self):
        ...
        for url in playlist_urls:
            assert is_spotify_playlist_url(url) is True, f"{url} should be detected as playlist"
        
        for url in non_playlist_urls:
            assert is_spotify_playlist_url(url) is False, f"{url} should not be detected as playlist"
    ...
    def test_is_spotify_album_url(self):
        ...
        for url in album_urls:
            assert is_spotify_album_url(url) is True, f"{url} should be detected as album"
        
        for url in non_album_urls:
            assert is_spotify_album_url(url) is False, f"{url} should not be detected as album"
    ...
    def test_is_spotify_track_url(self):
        ...
        for url in track_urls:
            assert is_spotify_track_url(url) is True, f"{url} should be detected as track"
        
        for url in non_track_urls:
            assert is_spotify_track_url(url) is False, f"{url} should not be detected as track"
    ...
    def test_get_spotify_url_type(self):
        ...
        for url, expected_type in test_cases:
            ...
            assert url_type == expected_type, f"Expected {expected_type}, got {url_type} for {url}"
