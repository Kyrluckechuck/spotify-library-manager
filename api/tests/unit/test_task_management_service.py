...
from api.src.graphql_types.models import MutationResult
from api.src.services.task_management import TaskManagementService
...
class TestTaskManagementService:
    ...
    @patch("api.src.services.task_management.HUEY")
    def test_get_task_count_by_name(
        self, mock_huey, task_management_service, mock_pending_task
    ):
        ...
        task3.name = "other_task"

        mock_huey.pending.return_value = [task1, task2, task3]

        result = task_management_service.get_task_count_by_name()

        assert result["test_task"] == 2
        assert result["other_task"] == 1

    @patch("api.src.services.task_management.HUEY")
    def test_get_task_count_by_name_not_found(self, mock_huey, task_management_service):
    ...
    @patch("api.src.services.task_management.HUEY")
    def test_cancel_all_pending_tasks_success(
        self, mock_huey, task_management_service, mock_pending_task
    ):
        ...
        result = task_management_service.cancel_all_pending_tasks()
    ...
    @patch("api.src.services.task_management.HUEY")
    def test_cancel_all_pending_tasks_empty_queue(
    ...
    @patch("api.src.services.task_management.HUEY")
    def test_cancel_all_pending_tasks_partial_failure(
        self, mock_huey, task_management_service
    ):
    ...
    @patch("api.src.services.task_management.HUEY")
    def test_cancel_all_pending_tasks_huey_exception(
        self, mock_huey, task_management_service
    ):
    ...
    @patch("api.src.services.task_management.HUEY")
    def test_cancel_tasks_by_name_success(self, mock_huey, task_management_service):
        ...
        result = task_management_service.cancel_tasks_by_name("test_task")
    ...
    @patch("api.src.services.task_management.HUEY")
    def test_cancel_tasks_by_name_not_found(self, mock_huey, task_management_service):
        """Test cancelling tasks by name when no matching tasks exist."""
        mock_huey.pending.return_value = []

        result = task_management_service.cancel_tasks_by_name("non_existent_task")
    ...
    @patch("api.src.services.task_management.HUEY")
    def test_cancel_tasks_by_name_partial_failure(
        self, mock_huey, task_management_service
    ):
    ...
    def test_get_queue_status_success(self, mock_huey, task_management_service):
        ...
        result = task_management_service.get_queue_status()
    ...
    @patch("api.src.services.task_management.HUEY")
    def test_get_queue_status_empty(self, mock_huey, task_management_service):
    ...
    @patch("api.src.services.task_management.HUEY")
    def test_get_queue_status_exception(self, mock_huey, task_management_service):
        """Test getting queue status when Huey raises an exception."""
        mock_huey.pending.side_effect = Exception("Huey error")

        # The actual implementation doesn't handle exceptions, so this should raise
        with pytest.raises(Exception, match="Huey error"):
            task_management_service.get_queue_status()
