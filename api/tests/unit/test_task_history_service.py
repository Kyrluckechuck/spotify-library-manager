...
from api.src.services.task_history import TaskHistoryService
from api.src.types.models import TaskType, EntityType, TaskStatus
from api.tests.factories import TaskHistoryFactory, CompletedTaskFactory, FailedTaskFactory
...
class TestTaskHistoryService:
    ...
    async def test_get_connection_with_sorting(self, task_history_service):
        ...
        await sync_to_async(TaskHistoryFactory)(started_at=datetime.now() - timedelta(hours=2), task_id='old_task_1')
        await sync_to_async(TaskHistoryFactory)(started_at=datetime.now(), task_id='new_task_1')
    ...
    async def test_get_connection_empty_result(self, task_history_service):
        ...
        items, has_next, total = await task_history_service.get_connection(
            first=10,
            status=TaskStatus.FAILED  # Filter by FAILED status
        )
    ...
    async def test_get_connection_with_cursor_pagination(self, task_history_service):
        ...
        # Get first page
        items1, has_next1, total1 = await task_history_service.get_connection(
            first=2
        )
